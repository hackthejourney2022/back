...
export async function loadConfiguration(): Promise<Config> {
  ...
  const newConfig: Config = {
    ...
    [ConfigTypes.amadeus]: {
      clientId: env.AMADEUS_CLIENT_ID ?? config[ConfigTypes.amadeus].clientId,
      clientSecret:
        env.AMADEUS_CLIENT_SECRET ?? config[ConfigTypes.amadeus].clientSecret,
    },
    [ConfigTypes.nominatim]: {
      url: env.NOMINATIM_CONFIG ?? config[ConfigTypes.nominatim].url,
      timeout:
        Number(env.NOMINATIM_TIMEOUT) ?? config[ConfigTypes.nominatim].timeout,
